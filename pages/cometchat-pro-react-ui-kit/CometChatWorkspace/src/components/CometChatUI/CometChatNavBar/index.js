...
//   }
  componentDidMount() {
	  const use = localStorage.getItem(":common_store/user");
	  const isCoachDB = use && JSON.parse(use).split(`"role":`)[1].split(`,"status`)[0].search("coach") !== -1;
	  this.setState({ isCoach: isCoachDB });
    const us = this.contextType;
